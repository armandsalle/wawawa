...
function verifyWebhook(...args: Parameters<Webhook["verify"]>) {
  return clerkWebhooksClient.verify(args[0], args[1]);
}

export const webhooksRouter = new Hono().all(
  "/clerk",
  ...
  async (c) => {
    ...
    verifyWebhook(JSON.stringify(rawBody), {
      "svix-id": svix_id,
